		...
		value: function saveColumnWidths() {
			...
			this.$tableHeaders.each(function (_, el) {
				...
				if (!$el.is(_constants.SELECTOR_UNRESIZABLE)) {
					localStorage.setItem(_this4.generateColumnId($el), _this4.parseWidth(el));
				}
			});
		}
	}, {
  ...
  **/
		value: function restoreColumnWidths() {
			...
			this.$tableHeaders.each(function (_, el) {
				...
				if (!$el.is(_constants.SELECTOR_UNRESIZABLE)) {
				    var width = localStorage.getItem(_this5.generateColumnId($el));

					if (width != null) {
						_this5.setWidth(el, parseInt(width));
					}
				}
			});
		}
